...
\hl{Recent years have seen an enormous trend to employ new technologies for satellite image processing and analysis. Advanced GIS software specially designed to process, model and interpret satellite images enable to display the landscapes of the Earth and to map the thematic maps on the basis of image classification. At the same time, a rising pool of the space data obtained from Earth Observation create a demand for cutting edge technologies. Up until now, processing satellite images was a task performed by GIS that employ specialised functions to capture, handle and analysis remote sensing data. Selected commercial software programs that operate with images are expensive and complicated to operate. Such restricted access has prevented the spread of such technologies, and thus limited  the use of proprietary software. As a response to these needs, the novelty of this study presents the use of advanced scripting approach of the open source GRASS GIS tool for processing remote sensing data.}
...
Inspired by the existing examples of the RS data analysis in environmental monitoring \cite{f9020059,Lemenkova202229,Fathalli,Abbas,Henchiri}, this study integrated the advanced methods of the GRASS GIS with applied techniques of its image processing modules for assessment and mapping of changes in landscapes of saline lakes in northern Tunisia, Figure \ref{fig01}. \hl{The main purpose of this study is to evaluate} the response \hl{of land cover types around salt lakes of Tunisia }to seasonal fluctuations in temperature \hl{and} the \hl{increase} of salinisation. The \hl{methodological approach aims at providing the} advanced RS and cartographic methods. Importantly, the study presented here is not intended to suggest that RS data replace the hydrological datasets. In contrast, these images can supplement the fieldwork survey in the sabkhas and environmental tests on the saline soil of Tunisia. 
...
Theoretically, the essential  principle of the land cover type classification is based on the fundamental properties of RS which consists in measurement of spectral reflectance as a reflection of light from the Earth's surface, as can be visible on the scenes, Figure \ref{fig04}. Spectral reflectance signatures of various land cover types (land, water, salt soils, vegetation, urban areas, etc) differs significantly. This is because of the different emitted radiation from these these objects which \hl{notably} varies. The Landsat 8-9 OLI/TIRS sensor continuously measures \hl{the }spectral reflectance of the objects on the Earth which are identified as various land cover types. Hence, the performed measurements indicate \hl{different} bodies and objects on the Earth using their spectral reflectance visible on the spaceborne satellite images in selected band combinations. Consequently, since measured land cover types \hl{vary}, it enables to perform classification of the land cover types using RS data processing \hl{method}, following by cartographic visualization and interpretation. 

Practically, the information on spectral reflectance is used for classification procedure. \hl{In this study, the method included a machine learning techniques which is based on a fully automated approach that is independent from human-based sampling. In this case, the partition of the image was relied on computer vision algorithms which determined the sampling categories and assigned pixels to classes automatically using programmed 'i.cluster' module of GRASS GIS. This approach differs significantly from the traditional supervised classification is based on training samples. }Thus, after clustering, the images were processed using special module of the GRASS GIS which performs unsupervised automatic classification of the satellite images. The advanced methods of GRASS GIS present effective instruments for classification -- the 'i.maxlik', \hl{originally }developed by M. Shapiro and T. Wen, and supported with additional module by M. Nartiss. The use of the 'i.maxlik' only required to specify the parameters of signature file and related information on groups and subgroups of spectral bands during data partition, and to assign pixels into each land cover class according to the records of the spectral reflectance in binary format. 

\hl{After the number of the defined classes was set up, the Landsat images were prepared for classification. The program identified land cover classes based on the cluster analysis that separates areas of contrast in a colour composite of image scenes and recognised the pixels that belong to this particular class. As a result, land cover classes were identified for which the appropriate pixels were assigned. }The spectral reflectance of wavelengths which develops proportionally with internal gain in strength in brightness of land cover types identified from space by the sensor. The technical characteristics of this module include two steps: the preceding clustering and following classification. The module is designed \hl{for} the multispectral images, such as Landsat, using spectral signatures of the pixels. 

The classification procedure was performed with different images (6 scenes for the study area \hl{1} in the Gulf of Gabès, and 6 images for the study area \hl{2} for the Gulf of Hammamet). The aim of the classification \hl{is }to identify cells in the classified image and to group \hl{them} into classes according to the similarity of spectral reflectance in each given group (class)\hl{. This is achieved using the }estimated spectral reflectance \hl{values by means of the } automatic \hl{algorithm of }image analysis\hl{. }As for comparison of \hl{the }several images taken on various seasons and\hl{ }years with \hl{the six} year \hl{time span}, the main \hl{objective} was to compare the effects of climate, the impact from the Sahara \hl{desert }and seasonal meteorological fluctuations on the \hl{increase} of salinisation in sabkhas of Tunisia\hl{. }Therefore, the experiments with images were implemented with different calendar seasons and in \hl{spatially distinct} regions (central/southern) of Tunisia. 

%----------- subsection ----------->

 \subsubsection{Accuracy analysis}

The images were evaluated for accuracy and pixels with high level of rejection were filtered out using 'r.mapcalc' module of GRASS GIS. The rejection probability values, i.e., the pixel classification confidence levels were computed and visualised and a series of maps. The error matrix and \hl{the }convergence level of classified pixels were measured using \hl{the }'i.cluster' module that includes accuracy assessment of classification results in tabular format. After that, the classification was performed \hl{and} the results of the clustering were exported as a series of matrices in CSV format for each images, and the results are reported in the tables in the \hl{auxiliary materials supporting }this study. \hl{The separability indicating was based on spectral reflectance properties of pixel's characteristics. The pixels on the image were assigning to categories of land cover types using threshold values which belong to this particular class and all other pixels assigned to other classes for automated detection of landscape patches. }

\hl{Because the GRASS GIS classifies remote sensing data based on their spectral characteristics and separability of land cover classes according to their categories, the quality of original images may affect the automatic recognition and interpretation of land and vegetation classes. Therefore, images were selected based on low cloudiness and free of atmosphere influence. }The classified satellite scenes were verified and class separability matrix was computed for each of the 12 images. The accuracy of the classification was measured and summarised in the tables with a convergence level of classified pixels above 98\% of points stable. Such approach to analyse the accuracy of image processing provided data to evaluate the obtained values of land cover classes at each image of the processed time series.
 
 %----------- subsection ----------->
 \subsubsection{Mapping}

The environmental properties of the landscapes were evaluated \hl{using} cartographic methods \hl{and} image processing\hl{, and}  visualised \hl{on the thematic }maps. The aim is to analyse the seasonal effects of climate\hl{, that is, the }increase of temperature and evaporation\hl{ }on the behaviour of salt lakes. The complete cartographic workflow used in this study included a sequence of the following modules of GRASS GIS:
...
These GRASS GIS modules have been incorporated to perform various steps of cartographic tasks and image processing workflow \hl{using the} console. The land cover types were identified continuously across the images using a sequence of k-means clustering and maximum likelihood classification approaches, and converted to the maps\hl{. The thematic maps indicated }variations in land cover types over Tunisia during a period of 2017 to 2023 for two image datasets covering the regions on the Gulf of Hammamet and the Gulf of Gabès. The data obtained from the GRASS GIS based image processing workflow were evaluated for environmental analysis.

%----------- section ----------->
\section{Results and Discussion}

\hl{The results present maps obtained from the GRASS GIS algorithms applied to a time series of satellite images on Tunisia and a comparison of changes in land cover types. }The results of the image processing approach of the GRASS GIS demonstrated in this paper show that the summer period \hl{intensifies} soil salinisation and mineralisation in sabkhas of Tunisia\hl{. This is } demonstrated through \hl{the presented }series of \hl{maps based on the images taken} on several seasons\hl{. Using} the information received from spectral reflectance of the pixels on the images and heat development in Sahara during summer months\hl{, the potential opportunity for the increase in} salinisation for each lake in various years \hl{was evaluated}. Figure \ref{fig05} demonstrates the results of the image classification showing the behaviour of saline lakes over seasonal between 2017 and 2023 in the region of Gulf of Hammamet (Sebkhet de Sidi el Hani, Sebkha de Moknine and Sebkhet El Rharra). 
...
Figure \ref{fig06} demonstrates the outputs of the image classification \hl{which show} variations in land cover types and saline lakes over seasonal between 2017 and 2023 in the region of Gulf of Gabès (Sebkhet en Noual). The land cover types were identified on the images using information\hl{ }modified \hl{from} the existing classification scheme on land cover types in Tunisia \cite{plants10102031} and data from Food and Agriculture Organization (FAO) \cite{FAO}. The existing land cover types for the country-level scale were \hl{adopted} according to the extent of the images covering study area and included the following types: 1) forest; 2) grasslands and rangelands; 3) sand and desert; 4) cultivated agriculture plantations and arable lands (olive tree, orchards, vine, palm groves); 5) urban areas and artificial land; 6) wasteland; 7) wetlands and water; 8) bare land and soil; 9) mixed vegetation; 10) saline soil. 

\hl{The variability of} seasons enabled to monitor and interpret how the salinisation process \hl{activates} the accumulation of minerals in the salt lakes with changed colour of sabkhas. \hl{During }summer period, the structure of the crystals in the soil sediments in sabkhas was solidified by the increased evaporation as a result of the increased temperatures. In turn, the seasonal effects of increased temperature and lack of precipitation in the southern region (Figure \ref{fig06}) are reflected in the increased areas of sand and declined vegetation, as shown in the maps. Changes in salt lakes were determined at different years during recent decade\hl{. This resulted from the comparative analysis of the }Landsat images taken in winter/spring/summer months. \hl{The difference illustrated the} periods \hl{of low and high }water inflow into Saharan lakes\hl{. The} decline in vegetation areas is clearly \hl{visible on} the images of 2017 from April to July showing sabkhas Sebkhet de Sidi el Hani, Sebkha de Moknine  and Sebkhet El Rharra, Figure \ref{fig06}. 
...
Varied temperature in seasonal climate periods has effects on the \hl{land cover type changes around} lakes and \hl{the level of }salinisation of sabkhas\hl{. }This \hl{is }also \hl{an} indirect indicator of \hl{the }groundwater inflow and \hl{the }impacts of \hl{the }coastal climate on the eastern sabkhas\hl{. For instance, this is typical }for \hl{Sebkha} de Moknine which is located near the Bekalta coastal town. This lake \hl{demonstrates} the least difference in the \hl{level of }salinisation\hl{, because it} was filled with water during the \hl{while year of }2017\hl{. In contrast, }in 2023 it experienced salinisation only during summer. \hl{If compared with} Sebkhet El Rharra\hl{, the latter} experiences clear \hl{and regular }fluctuations in the salinisation during each year being regularly filled with water in winter and covered by salt crust during July. Such changes were detected both for year 2017 and for year 2023.

The \hl{extend of the }agricultural areas \hl{is }more similar in Figure \ref{fig06} for the Gulf of Gabès area, \hl{which indicates} that the plantation areas in southern regions are\hl{ }similar for different landscape spots compared to the northern segment (Figure \ref{fig05}). This is also seen in Figure \ref{fig06} for 2017 and 2023 years on the month of April. The\hl{ }comparative analysis of several images showing \hl{the }dynamics of land cover types as descriptors \hl{demonstrated the effective and straightforward approach of RS data processing }to reveal the development of \hl{the }aridification and desertification in southern regions closer to Sahara in study areas. \hl{In particular, the use of GRASS GIS presents an} efficacious strategy to extract contextual information \hl{from the satellite images. This information can be used in environmental monitoring for analysis of }land cover properties\hl{. }\hl{Although the} RS data cannot directly detect landscape dynamics\hl{, }processing of \hl{the }RS data enables to approximate the extent of \hl{the }landscape patches from \hl{the }classified \hl{and analysed }images\hl{. It furthermore enables to} evaluate water content in salt lakes as indirect indicators of salinisation during \hl{the }summer heat \hl{period}.
...
\hl{The }analysis of the satellite images\hl{ shows }gradual changes in \hl{salt }lakes over \hl{the }calendar periods\hl{, } and provides a robust view in \hl{the }environmental changes. \hl{The }presented\hl{ }time series of \hl{the }processed\hl{ }images\hl{ }showed that the increase in salt lakes and desertification of surrounding areas is more distinct in the southern segment which showed that the Sebkhet en Noual is prone to salinisation even during the winter months\hl{. In contrast, }northern sabkhas Sebkhet de Sidi el Hani , Sebkha de Moknine and Sebkhet El Rharra show water-filled basins during winter \hl{period}. Furthermore, \hl{the results} demonstrated that the regions of Sfax are more prone to \hl{the }aridification compared to the areas of Mahdia and Sousse. 

\hl{The }changes in water level in salt lakes\hl{ }detected using \hl{the }RS data\hl{ }can serve for environmental monitoring of Tunisian landscapes and analysis of environmental consequences of climate effects in central and north Tunisia. \hl{The applications} of image processing \hl{for} environmental landscape \hl{analysis were}described earlier \hl{ based on various GIS}\cite{Touhami,Khelifa,Hammami,Bounouh}. \hl{Earlier} works use the Landsat MSS/4 and SPOT HRV/3 images \cite{Park}, Landsat and SRTM processed by ArcGIS and ENVI commercial software \cite{Souissi,Afrasinei} or a combination of ETM+ and OLI 8 sensors of the Landsat products \cite{Mezned}. \hl{In contrast, our} results\hl{ }are obtained using\hl{ recent }Landsat OLI/TIRS images processed by the GRASS GIS scripts. 

The reaction of \hl{minerals during }sand salinisation processes \hl{under effect of} high evaporation\hl{ }results in \hl{the }changed structure of pores of soil\hl{. This} significantly \hl{reduces} water content in soil\hl{ }during periodic occurrences of a deep brine layer on the bottom of the lakes. Such behaviour of soil as lacustrine sediment layer demonstrates the response to climate and meteorological processes so that the considerable fluctuations in water level altered the soil structure\hl{. This }is reflected as \hl{the }increased salinity and decreased water level, as evaluated during \hl{the images taken on }winter/spring/summer periods\hl{. }Moreover, the degree in salinisation of sabkhas well correlates with the location of each lake\hl{. For instance, } southern lakes \hl{are} almost completely covered by salt even during summer, while northern lakes demonstrate \hl{higher fluctuations}. Similar to \hl{the }existing studies \cite{BAYATI2021126032,SHENG2016129,SCHRODER20221710} \hl{in which} the salinisation of lacustrine soil \hl{was }evaluated using \hl{the }RS data, this study showed the highest level of changes \hl{during }July\hl{. Moreover, }the most contrasting behaviour of brine accumulation \hl{is found }in the lake of Sebkhet de Sidi el Hani (Sousse region)\hl{. This can be explained by the fact that this lake} has two \hl{interconnected} sub-basins with different morphology \hl{which} might have effects on \hl{the level of }salinisation of the lake. 

\hl{A }special advantage of this work is the use of the open source Landsat OLI/TIRS data and a free GRASS GIS software \hl{for  environmental monitoring of sabkhas in Tunisia}. \hl{The advanced scripting cartographic approach of GRASS GIS} was introduced to evaluate the lacustrine properties of soil around the sabkhas in north Tunisia using RS data processing. It is well known that proprietary GIS and RS software are expensive since their development is costly (e.g., ArcGIS or Erdas Imagine). Moreover, very high resolution satellite images are not always easily accessible since they are provided on the commercial basis (e.g., SPOT, IKONOS). This makes the present work repeatable, which can be continued in similar relevant studies in the future projects. The presented open source solutions on the optimal selection of data and tools can be supportive for environmental monitoring  of Tunisia. \hl{A }series of the satellite images were processed with various combination of GRASS GIS modules and compared with the images covering identical areas on earlier periods\hl{. }The performed experiments on RS data analysis supported monitoring of fluctuations in water level of salt lakes of Tunisia through the analysed time series analysis of \hl{the }satellite images. 
...
\hl{The }GRASS GIS software has been successfully applied to identify different stages \hl{in }landscape change in sabkhas of north Tunisia \hl{with} a special focus on several lakes -- Sebkhet de Sidi el Hani, Sebkha de Moknine, Sebkhet El Rharra and Sebkhet en Noual\hl{. }The study also evaluated the effects of climate on seasonal changes in the lacustrine landscapes, as indicated by \hl{the }development of land cover types over the period of \hl{2017 to 2023}. Besides, the experiments served as a means of evaluating \hl{climate} effects as accelerators of salinisation\hl{ in }sediments of lakes. The presented results confirm that GRASS GIS can be effectively used to processing and classification of \hl{the }RS imagery using programming \hl{methods} that ensures automation\hl{, }precision and accuracy of data \hl{analysis}. 

\subsection{Recommendations}

For future similar works on RS data analysis of salt lakes, \hl{the} following recommendations and technical suggestions \hl{are provide }for mapping and image analysis:
...
\begin{enumerate}
	...
	\item Due to the dynamics of plant phenology in different months, the period of image capture should be identical to evaluate changes for each image. For example, \hl{correct} comparison can be performed using images taken for a specific month\hl{, e.g.,} January, in a sequence of several years\hl{. Alternatively, use} several identical months for two years, as presented in this study.
	\item Cloudiness vary a lot in different images. It is necessary to use a cloud-free series of images as\hl{ reliable input} dataset. While in this study, 10\% of cloud coverage was applied, the modified parameters optimised with different cloud content can be tested and applied with decreased cloud coverage. In this way, the use of 'i.landsat.acca' module of the \hl{GRASS} GIS enables Automatic Cloud Cover Assessment (ACCA) for a quick evaluation of image quality and suitability.
	\item Process other dataset in addition to the Landsat, e.g., Advanced Spaceborne Thermal Emission and Reflection Radiometer (ASTER) images. ASTER data can be \hl{processed} using special modules of 'i.aster.toar', similar to the 'i.landsat.toar'. The main advantage of ASTER is a higher resolution \hl{with }a pixel size of 15 m compared to the 30-m Landsat. 
	\item For mapping, take satellite images from the USGS on diverse dates to evaluate spatio-temporal dynamics and perform mapping \hl{based }on images captured on different seasonal periods to compare the difference in salinisation between the lakes.
	...
	\item Evaluate the integral effects from various factors that \hl{might cause land cover} changes, i.e., the heat from the increased temperature, decreased precipitation and high evaporation lead to desertification of landscapes. 
	\item The access to the groundwaters and geographic location of the landscapes affect the \hl{response of} vegetation. For instance, closeness to the desert or coastal areas affect differently\hl{ patterns of plants in the} landscapes. 
	\item For analysis of vegetation health and vigour, the computation of the NDVI or other vegetation indices is a useful means of environmental assessment. In GRASS GIS, vegetation indices can be automatically calculated \hl{using} the embedded formulae adjusted to the bands of the \hl{image}. For example, NIR and Red channels correspond to the bands 5 and 4 for the Landsat 8-9 OLI/TIRS, while bands 4 and 3 for older Landsat 7 ETM+ images.
	\item Changes in land cover types within certain limits indicate \hl{landscape} dynamics as a response to the environmental and climate effects and processes. Moreover, it may indicate the cumulative effects of several factors on soil-vegetation conditions, including both climate-environmental processes and anthropogenic activities.
\end{enumerate}

\subsection{Concluding remarks}

This paper reports the results obtained \hl{of the} experimental investigations on satellite images Landsat 8-9 OLI/TIRS collected from the USGS \hl{repository}. The tests on image processing were designed to evaluate\hl{ changes in land cover types} in several saline lakes of north Tunisia in various seasons in \hl{2017} to 2023\hl{, as well as the }variations in water extent as indicators of salinisation. Specifically, we evaluated the proposed framework on RS data analysis using GRASS GIS software and \hl{Landsat} series. We also proposed future directions in similar works to develop robust scheme on RS data processing\hl{ in} similar studies where cartographic approach to image analysis is applied in environmental monitoring projects aimed at evaluation of climate effects on landscape changes. In this work\hl{, }we also discussed the ways in which\hl{ scripting} approach of cartographic data processing contrasts with traditional GIS methods of mapping that use Graphical User Interface (GUI), and complement to RS data processing for evaluating the connections between the climate effects and salinisation of sabkhas in Tunisia. 

\hl{The} advantages of the GRASS GIS \hl{were demonstrated }as a quantitative, robust and advanced cartographic tool for evaluating environmental properties based on image analysis. A workflow has been developed \hl{for} a series of high-resolution Landsat 8-9 OLI/TIRS images with various combinations of GRASS GIS modules\hl{. The data were }evaluated to analyse the properties of lacustrine landscapes of north Tunisia as a function of summer heat in Sahara and increase in evaporation during the period of \hl{5} years (\hl{2017}-2023). The approach of GRASS GIS demonstrated \hl{a }more advanced way of image processing compared to traditional GIS software due to scripts that enables automation of workflow repeatability of the process. \hl{The} use of several module of GRASS GIS ensures multifold manipulations with images, \hl{the} 'i.segment', 'i.maxlik' and other sub-tests\hl{ were} applied for various steps of multispectral image analysis. 

A scripting approach of the GRASS GIS \hl{supported mapping and analysis of the} rate of salinisation in sabkhas of Tunisia \hl{in} various seasons from \hl{2017} to 2023 and to obtain the information from the processed images representing the land cover types in the surroundings. In turn, \hl{the }RS datasets enabled to model the behavior of landscapes in spring, summer and winter periods in north Tunisia which was used to compare the climate effects from rise in temperature on the evaporation of water in salt lakes, as directly related to heat increase during \hl{the }summer period in Sahara. The results demonstrated variations in water level in several sabkha and show technical effectiveness of the GRASS GIS in cartographic processing of the RS data for environmental tasks. \hl{The }correlations between the increase in heat in \hl{different} years and high salinisation of sabkhas were detected. The \hl{presented }image processing \hl{has} investigated the salinisation behaviour in saline lakes with thermal variations using \hl{the }analysis of Landsat bands in multispectral channels. 

Due to the effectiveness and applicability of the performed experiments on image analysis by GRASS GIS -- flexibility of scripting approach and a wide variety of modules tuned for diverse tasks of image processing using its modular approach -- we expect the proposed method to be able to deal with other satellite imagery\hl{. This can be used to study} diverse regions of north Africa \hl{and analysis the }development of lacustrine landscapes with effects from climate and fluctuations in temperature on Saharan ecosystems. Various sabkhas of north Tunisia with diverse surrounding soil types and salinisation \hl{or} mineralisation degree were evaluated to generalise the proposed workflow of salt lake monitoring to a higher dimensional workload in the larger lakes and different\hl{ }ecosystems with similar salt lakes. 

As demonstrated in this study, the RS data from the USGS EarthExplorer repository can be used to analyse the variations in salt lakes. It enables to derive information using data on spectral reflectance and brightness of pixels which strongly correlates with the distribution of various land cover types. In such a way, remote sensing data processing enables to detect patches in complex diversified lacustrine landscapes of north Sahara characterised with heterogeneous environment in \hl{a} contrasting climate of desert. This paper contributes to the environmental monitoring of Sahara, analysis of salt lake variations and regional studies of Tunisia, north Africa.
