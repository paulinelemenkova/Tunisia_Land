...
After the classification the maps of the land cover types were visualised and accuracy assessment was determined by statistical analysis in GRASS GIS. The class separability matrices were computed based on the nature of the maximum likelihood algorithm which assigns pixels into classes along with the increase of the similarity of spectral reflectance in cells as estimating the probability distribution in the observed dataset of pixels. Thus, in a statistical evaluation of the accuracy of this approach, the probability of the assignment of the pixels to a specific land cover class is reflected in the increasing closeness of the value of pixels indicating spectral reflectance of pixels detected through the complete image. Nevertheless, certain pixels might be misclassified due to the individual properties of objects that enable them to misclassify to the erroneous classes due to a very similar level of spectral reflectance. To this end, accuracy of the assigned pixels to a given class was evaluated that estimates the specificity and sensitivity of pixels within the given land category as well as positive and negative predictive values. This included computed means and standard deviation for each band of the satellite images, computed class separability matrices, and plotted reject probability maps for each of the 12 maps. The results of the calculations are presented in the Appendix \ref{AppB} separately for each of the 12 processed images.
